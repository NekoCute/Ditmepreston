...
function Automap()
    while task.wait() and Config.automap do
        ...
        function()
            ...
            if Library["MapCmds"].GetCurrentZone() ~= Zone_Info._id or not Library["MapCmds"].IsInDottedBox() then
                if Zone_Info.ZoneFolder:FindFirstChild("INTERACT") then
                    ...
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                        Zone_Info.ZoneFolder.PERSISTENT.Teleport.CFrame
                end
            end
        end
    )
...
local GetNearestBreakable =
    getsenv(game:GetService("Players").LocalPlayer.PlayerScripts.Scripts.GUIs["Auto Tapper"]).GetNearestBreakable
